...
class MultiGoalListen():
    def __init__(self):
        ...
        self.map_pose_file="/home/eaibot/dashgo_ws/src/dashgo/dashgo_nav/maps/maps_pose.json";
        if os.path.exists(self.map_pose_file)==False:
            open(self.map_pose_file,"w")
        self.initPose=False
        rospy.Subscriber('/robot_pose', Pose, self.robot_pose_callback)
        rospy.Subscriber('/initialpose', PoseWithCovarianceStamped, self.init_robot_pose_callback)
    ...
    def robot_pose_callback(self, msg):
        if self.initPose==True :
            self.count=self.count+1
            if self.count==5:
                self.count=0
                attr_dict = message_converter.convert_ros_message_to_dictionary(msg)
                maps=json_file.json_read_lines(self.map_pose_file)
                count=0
                isSave=False
                for one_map in maps:
                    if one_map[u"isUsed"]==True:
                        isSave=True
                        maps[count][u"pose"]=attr_dict
                        break
                    count=count+1
                if isSave==True:
                    json_file.json_clear(self.map_pose_file)
                    for one_map in maps:
                        json_file.json_append(self.map_pose_file, one_map)
            
    def init_robot_pose_callback(self, msg):
        self.initPose=True
