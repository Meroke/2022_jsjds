  ...
  //, angular_velocity_( 0 )
{
  ...
  //到达目标点后后退的距离
  QHBoxLayout* back_dist_1_layout = new QHBoxLayout;
  back_dist_1_layout->addWidget( new QLabel( "back_dist:" ));
  back_dist_1_editor_ =new QLineEdit;
  back_dist_1_layout->addWidget( back_dist_1_editor_ );
  ...
  pose_2_layout->addWidget(pose_2_editor_);
    ...
    //到达目标点后后退的距离
  QHBoxLayout* back_dist_2_layout = new QHBoxLayout;
  back_dist_2_layout->addWidget( new QLabel( "back_dist:" ));
  back_dist_2_editor_ =new QLineEdit;
  back_dist_2_layout->addWidget( back_dist_2_editor_ );
...
void MoveToGrab::GetRobotPoseCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
 ...
 // ROS_WARN("robot pose= (%f,%f,%f)",current_pose_.position.x, \
  ...
  //qDebug()<<"robot pose= ("<<current_pose_.position.x<<","<<current_pose_.position.y \
         <<","<<current_pose_.position.z<<")";
}
...
void MoveToGrab::getPose1Data(move_to_grab::MoveToGrabMsg& pose1_msg)
{
    ...
    qDebug()<< back_dist_1_editor_->text();
    if(back_dist_1_editor_->text() == " ")
    {
        //qDebug()<<"distance is " ",wei 0 ";
        pose1_msg.backDistance.data=0;
    }
    else
    {
        pose1_msg.backDistance.data=back_dist_1_editor_->text().toFloat();
        //qDebug()<<"distance is "<<pose1_msg.backDistance.data;
    }
}
...
#endif
    //pose1_msg=move_to_grab::MoveToGrabMsg(0);
    getPose1Data(pose1_msg);
...
void MoveToGrab::getPose2Data(move_to_grab::MoveToGrabMsg& pose2_msg)
{
    ...
    //qDebug()<<dobot_2_action_->currentText();
    //qDebug()<< back_dist_2_editor_->text();
    if(back_dist_2_editor_->text() == " ")
    {
        //qDebug()<<"distance is " ",wei 0 ";
        pose2_msg.backDistance.data=0;
    }
    else
    {
        pose2_msg.backDistance.data=back_dist_2_editor_->text().toFloat();
        //qDebug()<<"distance is "<<pose1_msg.backDistance.data;
    }
}
...
void MoveToGrab::save( rviz::Config config ) const
{
  ...
  //QString back_dist_1_str,back_dist_2_str;
  config.mapSetValue("back_dist_1",back_dist_1_editor_->text());
  config.mapSetValue("back_dist_2",back_dist_2_editor_->text());
...
void MoveToGrab::load( const rviz::Config& config )
{
  ...
  if( config.mapGetString( "back_dist_1", &back_dist_1_str ))
  {
    back_dist_1_editor_->setText(back_dist_1_str);
  }
  
  if( config.mapGetString( "back_dist_2", &back_dist_2_str ))
  {
    back_dist_2_editor_->setText(back_dist_2_str);
  }
