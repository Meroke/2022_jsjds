...
namespace serial {
    ...
    int Serial::waitforrecv(uint32_t timeout, std::size_t *returned_size)
    {
        return pimpl_->waitforrecv(timeout, returned_size);
    }

    int Serial::waitforsent(uint32_t timeout, std::size_t *returned_size)
    {
        return pimpl_->waitforsent(timeout, returned_size);
    }
    ...
    void Serial::setTimeoutMs(int timeout) {
        serial::Timeout time = serial::Timeout::simpleTimeout(timeout);
        pimpl_->setTimeout (time);
    }
